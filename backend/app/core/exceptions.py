...
class DockerAPIError(DockWatchException):
    """Raised when Docker API operation fails."""

    def __init__(self, message: str = "Docker API error"):
        super().__init__(
...
def setup_exception_handlers(app: FastAPI):
    ...
    async def dockwatch_exception_handler(request: Request, exc: DockWatchException):
        logger.error(
            f"DockWatchException: {exc.message}", extra={"details": exc.details}
        )
        return JSONResponse(
