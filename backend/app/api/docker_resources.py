...
async def list_images(
    ...
    """List Docker images"""
    try:
        docker = DockerClientService.get_client()
...
async def pull_image(
    ...
    try:
        ...
        if background_tasks:
            ...
            return {"status": "pending", "message": f"Pulling {image_name}:{tag} in background"}
        ...
        return [
            {
                ...
                "size": vol.attrs.get("UsageData", {}).get("Size", 0) if vol.attrs.get("UsageData") else 0
            }
            for vol in volumes
        ]
    except Exception as e:
        ...
        return [
            {
                ...
                "containers": list(net.attrs.get("Containers", {}).keys()) if net.attrs.get("Containers") else []
            }
            for net in networks
        ]
    except Exception as e:
