    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with tarfile.open(backup_path, "w:gz") as tar:
                ...
                config_path = Path(app_settings.CONFIG_PATH)
...
async def restore_backup(
    ...
    if not file.filename.endswith('.tar.gz'):
        raise HTTPException(status_code=400, detail="Invalid file format. Expected .tar.gz")
            ...
            for host_data in data.get("hosts", []):
                ...
                if not existing:
                    host = Host(
                        id=host_data["id"],
                        name=host_data["name"],
                        socket_path=host_data.get("socket_path", "unix:///var/run/docker.sock"),
